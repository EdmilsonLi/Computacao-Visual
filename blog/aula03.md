...
Representada pela equação s = c &middot; log(1 + r), essa função possui um comportamento bem específico:
...
Essa técnica é essencial para visualizar dados com alta variação dinâmica, como o **Espectro de Fourier**, onde a amplitude dos valores varia drasticamente (de 0 a 10<sup>6</sup>) e uma exibição linear comum esconderia quase todos os detalhes.

---

## 🎛️ Transformação de Potência (Lei de Potência / Gama)

Modelada pela fórmula s = c &middot; r<sup>&gamma;</sup>, o parâmetro &gamma; (gama) define como a curva de iluminação se comporta:
