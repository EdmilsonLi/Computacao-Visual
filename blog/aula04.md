...
Na filtragem espacial, cada pixel da nova imagem é calculado com base em uma região contígua ao seu redor (como uma janela 3x3). O *kernel* desliza sobre a imagem de entrada e combina os valores dos vizinhos.
...
* **Filtro da Média (Box Filter):** Substitui o pixel central pela média aritmética dos seus vizinhos (com pesos iguais a 1/9 em um kernel 3x3). Aumentar o tamanho da janela (ex.: 5x5 ou 17x17) intensifica o nível de suavização.
* **Filtro Gaussiano:** Utiliza uma média ponderada onde os pixels mais próximos do centro têm maior peso. Isso produz um borramento visualmente mais natural e preserva melhor as estruturas da cena.
